                                                                           ...
                                                                           ::std::byte const* instruction_end)
{
    ...
    auto const& local_types{state.local_types};
    auto& llvm_context{*state.llvm_context_holder};
    auto* llvm_module{state.llvm_module.get()};
    auto* llvm_function{state.llvm_function};
    auto& ir_builder{*state.ir_builder};
    auto const& local_pointers{state.local_pointers};
    ...
    auto& branch_target_stack{state.branch_target_stack};
    ...
    auto const prepare_wasm_call_operands{
        [&](::uwvm2::uwvm::runtime::storage::wasm_binfmt1_final_function_type_t const& wasm_function_type) -> llvm_jit_prepared_wasm_call_operands_t
        {
            llvm_jit_prepared_wasm_call_operands_t prepared{};
            prepared.abi_layout = get_runtime_wasm_call_abi_layout(wasm_function_type);
            if(!prepared.abi_layout.valid || operand_stack.size() < prepared.abi_layout.parameter_count) [[unlikely]] { return prepared; }
            ...
            {
                auto const argument{operand_stack.back()};
    ...
    auto const push_wasm_call_result{[&](llvm_jit_prepared_wasm_call_operands_t const& prepared, ::llvm::Value* value) -> bool
                                     {
                                         if(!prepared.has_result) { return true; }
                                         if(value == nullptr) [[unlikely]] { return false; }
                                         push_operand(prepared.result_type, value);
                                         return true;
                                     }};

    auto const emit_direct_wasm_call_value{
        [&](::uwvm2::uwvm::runtime::storage::wasm_module_storage_t const& runtime_module,
            validation_module_traits_t::wasm_u32 func_index,
            ::uwvm2::uwvm::runtime::storage::wasm_binfmt1_final_function_type_t const& wasm_function_type,
            ::llvm::ArrayRef<::llvm::Value*> call_arguments) -> ::llvm::CallInst*
        {
    ...
    auto const truncate_operand_stack_to{[&](::std::size_t target_size)
                                         {
                                             while(operand_stack.size() > target_size) { operand_stack.pop_back(); }
                                         }};

    auto const create_control_end_branch_target{[&](::std::size_t control_stack_index) noexcept
                                                {
                                                    auto const& target_context{control_stack.index_unchecked(control_stack_index)};
                                                    return llvm_jit_branch_target_t{.params = target_context.result,
                                                                                    .block = target_context.end_block,
                                                                                    .phi = target_context.end_phi,
                                                                                    .control_stack_index = control_stack_index};
                                                }};

    auto const mark_target_block_has_incoming{[&](llvm_jit_branch_target_t const& target)
                                              {
                                                  if(target.control_stack_index >= control_stack.size()) [[unlikely]] { return; }
                                                  auto& target_context{control_stack.index_unchecked(target.control_stack_index)};
                                                  if(target_context.end_block == target.block) { target_context.end_block_has_incoming = true; }
                                              }};

    auto const try_get_branch_value{[&](runtime_block_result_type params, llvm_jit_stack_value_t& branch_value)
                                    {
                                        auto const arity{get_runtime_block_result_count(params)};
                                        if(arity == 0uz)
                                        {
                                        ...
                                        if(arity != 1uz || operand_stack.empty()) [[unlikely]] { return false; }

                                        branch_value = operand_stack.back();
                                        return branch_value.value != nullptr && branch_value.type == get_runtime_block_single_result_type(params);
                                    }};

    auto const emit_branch_to_target{[&](llvm_jit_branch_target_t const& target)
                                     {
                                         auto* current_block{ir_builder.GetInsertBlock()};
                                         if(current_block == nullptr || current_block->getTerminator() != nullptr) [[unlikely]] { return false; }

                                         llvm_jit_stack_value_t branch_value{};
                                         if(!try_get_branch_value(target.params, branch_value)) [[unlikely]] { return false; }
                                         ...
                                         ir_builder.CreateBr(target.block);
                                         mark_target_block_has_incoming(target);
                                         return true;
                                     }};

    auto const branch_to_control_context_end{[&](::std::size_t control_stack_index)
                                             { return emit_branch_to_target(create_control_end_branch_target(control_stack_index)); }};

    auto const enter_unreachable_control_context{[&]()
                                                 {
                                                     if(control_stack.empty()) [[unlikely]] { return; }
                                                     auto& current_context{control_stack.back()};
                                                     truncate_operand_stack_to(current_context.outer_stack_size);
                                                     current_context.is_reachable = false;
                                                     unreachable_control_depth = 0uz;
                                                 }};
    ...
    auto const emit_runtime_raw_host_bridge_call{
        [&](::uwvm2::uwvm::runtime::storage::wasm_binfmt1_final_function_type_t const& wasm_function_type,
            ::llvm::ArrayRef<::llvm::Value*> call_arguments,
            char const* param_buffer_name,
            char const* result_buffer_name,
            auto&& emit_bridge_call_from_buffers) -> llvm_jit_runtime_raw_bridge_emit_result_t
        {
    ...
    auto const emit_local_imported_global_get_bridge_call{
    ...
    auto const emit_local_imported_global_set_bridge_call{
    ...
    switch(curr_opbase)
    {
        // Follow WAVM's structured-control lowering shape:
        // maintain an explicit control stack, branch labels as basic blocks, and skip-emission in unreachable regions.
#define UWVM_LLVM_JIT_EMIT_OPCODE_CASES
#include "opcode/control_flow_cases.h"
#undef UWVM_LLVM_JIT_EMIT_OPCODE_CASES
#define UWVM_LLVM_JIT_EMIT_OPCODE_CASES
