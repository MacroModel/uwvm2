...
{
    {
        ...
        if(sl::get_result<wasm_i32>(exec) != 11) { return 3; }

        args[0] = 0u;
        if(!sl::execute_function(compiled.function, args, exec)) { return 4; }
        if(sl::get_result<wasm_i32>(exec) != 22) { return 5; }
    }

    {
        ...
        if(sl::get_result<wasm_i32>(exec) != 9) { return 8; }

        args[0] = 1u;
        if(!sl::execute_function(compiled.function, args, exec)) { return 9; }
        if(sl::get_result<wasm_i32>(exec) != 0) { return 10; }
    }
...

    {
        ...
        constexpr value_type results[]{value_type::i32};
        constexpr sl::local_entry_t locals[]{sl::local_entry_t{.count = 1u, .type = value_type::i32}};
        sl::function_descriptor_t desc{.parameters = {}, .results = results, .locals = locals, .expression = expr};
        auto const compiled{sl::compile_function(desc)};
        if(!compiled) { return 14; }

        sl::execution_result_t exec{};
        ::std::array<wasm_u64, 0> args{};
        if(!sl::execute_function(compiled.function, args, exec)) { return 15; }
        if(sl::get_result<wasm_i32>(exec) != 7) { return 16; }
    }
