...
namespace
{
    ...
    [[nodiscard]] int run_suite(runtime_module_t const& rt) noexcept
    {
        ...
        constexpr bool f32_stacktop_disabled{Opt.f32_stack_top_begin_pos == SIZE_MAX || Opt.f32_stack_top_begin_pos == Opt.f32_stack_top_end_pos};
        constexpr bool f64_stacktop_disabled{Opt.f64_stack_top_begin_pos == SIZE_MAX || Opt.f64_stack_top_begin_pos == Opt.f64_stack_top_end_pos};
        if constexpr(Opt.is_tail_call && Opt.i32_stack_top_begin_pos == Opt.i32_stack_top_end_pos && f32_stacktop_disabled && f64_stacktop_disabled)
        {
    ...
    [[nodiscard]] int test_translate_br_if_float_cmp_localget_rhs() noexcept
    {
        ...
        // Mode D: tailcall + stacktop caching with scalar-all-merged stack-top window.
