...
namespace
{
    ...
    [[nodiscard]] byte_vec build_fv_ring_full_mixed_local_module()
    {
        module_builder mb{};
        ...
        // f0: Fill a two-slot FV window with three f32 values, then write an i32 local
        // before spilling the f32 values into locals.
        {
            func_type ty{{k_val_i32, k_val_f32, k_val_f32, k_val_f32}, {k_val_f32}};
            func_body fb{};
            fb.locals.push_back({1u, k_val_i32});  // local4
            fb.locals.push_back({3u, k_val_f32});  // local5..7
            auto& c = fb.code;
            ...
            op(c, wasm_op::local_set); u32(c, 4u);
        ...
        // f1: Same pressure pattern for f64 with an i64 local write in the middle.
        {
            func_type ty{{k_val_i64, k_val_f64, k_val_f64, k_val_f64}, {k_val_f64}};
            func_body fb{};
            fb.locals.push_back({1u, k_val_i64});  // local4
            fb.locals.push_back({3u, k_val_f64});  // local5..7
            auto& c = fb.code;
        ...
        // f2: Mixed f32/f64 values share the same FV physical window. This catches
        // per-type fullness accounting that forgets the shared physical slots.
        {
        ...
        // f3: local.tee for the integer value must not disturb the already full FV window.
        {
            func_type ty{{k_val_i32, k_val_f32, k_val_f32, k_val_f32}, {k_val_f32}};
            func_body fb{};
            fb.locals.push_back({1u, k_val_i32});  // local4
            ...
            op(c, wasm_op::local_get); u32(c, 3u);
            op(c, wasm_op::local_get); u32(c, 0u);
            op(c, wasm_op::local_tee); u32(c, 4u);
            op(c, wasm_op::drop);
            op(c, wasm_op::local_set); u32(c, 7u);
            op(c, wasm_op::local_set); u32(c, 6u);
            op(c, wasm_op::local_set); u32(c, 5u);
    ...
    [[nodiscard]] int run_suite(runtime_module_t const& rt, bool expect_fv_spill) noexcept
    {
        ...
        if(expect_fv_spill)
        {
            ...
            UWVM2TEST_REQUIRE((contains_typed_spill1<Opt, wasm_f32>(cm.local_funcs.index_unchecked(3).op.operands)));
        }
...

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(0),
                                  ...
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(load_f32(rr.results) == wasm_f32{17.5f});
        }

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(1),
                                  rt.local_defined_function_vec_storage.index_unchecked(1),
                                  pack_values(static_cast<wasm_i64>(20), wasm_f64{1.25}, wasm_f64{2.5}, wasm_f64{4.25}),
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(load_f64(rr.results) == wasm_f64{28.0});
        }

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(2),
                                  ...
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(load_f64(rr.results) == wasm_f64{17.0});
        }

        {
            auto rr = Runner::run(cm.local_funcs.index_unchecked(3),
                                  rt.local_defined_function_vec_storage.index_unchecked(3),
                                  pack_values(static_cast<wasm_i32>(6), wasm_f32{0.5f}, wasm_f32{1.25f}, wasm_f32{2.75f}),
                                  nullptr,
                                  nullptr);
            UWVM2TEST_REQUIRE(load_f32(rr.results) == wasm_f32{10.5f});
        }

        return 0;
    }

    [[nodiscard]] int test_translate_fv_ring_full_mixed_local() noexcept
    {
        install_unexpected_traps();
        optable::call_func = strict_terminate_call;
        optable::call_indirect_func = strict_terminate_call_indirect;

        auto wasm = build_fv_ring_full_mixed_local_module();
        auto prep = prepare_runtime_from_wasm(wasm, u8"uwvm2test_fv_ring_full_mixed_local");
...
int main()
{
    return test_translate_fv_ring_full_mixed_local();
}
