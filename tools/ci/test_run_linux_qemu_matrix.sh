...
if [[ ${1:-} == --version ]]; then
    ...
    case $profile in
        ...
        full-live|full-bad|full-aux-authority) printf 'Call Stack Modes Support: instruction, unwind, unwind-uncheck\n' ;;
...
if [[ ${1:-} == --help && ${2:-} == runtime ]]; then
    if [[ $profile == int-* ]]; then
        ...
        case $profile in
            ...
            full-live|full-bad|full-aux-authority) printf 'Usage: -Rllvm-call-stack [auto|instruction|none|unwind|unwind-uncheck]\n' ;;
...
if [[ $profile == int-* ]]; then
    ...
    if [[ $policy == auto ]]; then
        case $profile in
            ros-instruction) effective=instruction ;;
            full-live|full-bad|full-aux-authority) effective=unwind ;;
